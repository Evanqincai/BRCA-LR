...
data <- data[,c(1,2,4,9,11,12,13)]
colnames(data) <- c("chr","start","gene","Z_Score","up","down","Status")
p<- ggplot(data,aes(start/1000000,Z_Score)) + geom_point(aes(shape = Status,colour = Status)) + facet_wrap(~chr,scales = "free_x") + geom_line(aes(x = data$start/1000000 , y = data$up),color = "red",linetype = 1) + geom_path(aes(x = data$start/1000000 , y = data$down),color = "red",linetype = 1) + xlab("Pos(M)")  
