...
def read_bed(infile):
    ...
    def _parse_line(line):
        ...
        enID = (fields[4].rstrip() if len(fields) >= 5 else '-')
        strand = (fields[5].rstrip() if len(fields) >= 6 else '.')
        return chrom, int(start), int(end), gene, enID, strand
    with open(infile, 'rb') as handle:
        rows = map(_parse_line, (line for line in handle))
        return pd.DataFrame.from_records(
            rows,
            columns=["Chr", "start", "end", "gene", "refseqID", "strand"]
        )
...
def read_bed4(infile):
    ...
    table2 = table.loc[:, ['Chr', 'start', 'end', 'gene']]
...
def multi_bam_coverage(para):
    ...
    index1 = data_table[0].astype(str).str.cat(data_table[1].astype(str), sep='\t').str.cat(data_table[2].astype(str), sep='\t').str.cat(data_table[3].astype(str),sep='\t')
...
def multicov(bed="/work-a/user/guoh/Data/panel6.bed",
    ...
    data_table.insert(0,'c',data_table.index.map(lambda x:int(x.split('\t')[2])))
    data_table.insert(0,'b',data_table.index.map(lambda x:int(x.split('\t')[1])))
    ...
    if 'depth' in data_table:
        ...
        data_table.loc[ok_idx, sample_name+'-log2_ND'] = (
            np.log2(data_table.loc[ok_idx, 'depth']) -
            np.median(np.log2(data_table.loc[ok_idx, 'depth'])))
        tmp_GA = GenomicArray(data_table)
        tmp_GA.sort()
        ...
        for sample in col_names[4:]:
            data_table.loc[data_table[sample] == 0, sample] = 0.001
            ok_idx = (data_table[sample] > 0)
            data_table.loc[ok_idx, sample+'-log2_ND'] = (
                np.log2(data_table.loc[ok_idx, sample]) -
                np.median(np.log2(data_table.loc[ok_idx, sample])))
            tmp_GA = GenomicArray(data_table)
            tmp_GA.sort()
            gc, rmask = get_fasta_stats(tmp_GA, fa_fname)
            exon_len = np.array(tmp_GA.data['end']-tmp_GA.data['start'])
...
def ref_GCS(ref_Z, bamfs):
    ...
    ref_gcs['cut_off'] = ref_gcs.apply(
        lambda x: abs(x).sort_values(ascending=True)[int(0.95*len(x))], axis=1)
    return ref_gcs
...
def _cmd_autocall(args):
    ...
    ensure_path(fname4)
    SCNA_passFilter = CNV_result.query("(Exon_counts>5.0) & (Filter=='PASS')")
...
def _cmd_scatter(args):
    ...
    ref_data = ref_GA.data
    sample_data.index = list(
        sample_data.Chr.astype(str).str.cat(sample_data.start.astype(str), sep=':').astype(str).str.cat(
            sample_data.end.astype(str), sep='-'))
    ref_data.index = list(ref_data.Chr.astype(str).str.cat(ref_data.start.astype(str), sep=':').astype(str).str.cat(
        ref_data.end.astype(str), sep='-'))
    SN = int((len(ref_data.columns) - 7) / 2)
    result = pd.concat([ref_data.iloc[:, 0:4], ref_data.iloc[:, SN + 4:], sample_data.iloc[
                                                                          :, -1]], axis=1, join_axes=[ref_data.index])
    ...
    # ref_list = list(cc.columns[4:34]) + [str(cc.columns[-1])]
    # sample_list =  list(cc.columns[-2:])+['gene']
    sample_list2 = ['Chr', 'start', 'end', 'gene', 'tumor', 'id']
    pdata = cc[sample_list2]
    pdata2 = pdata.copy()
    pdata2['median'] = pdata2['gene'].map(dict(pdata2.groupby("gene").apply(lambda subg: subg['tumor'].median())))

    pass_dic = {}
    with open(SCNA_pass, 'rb') as f:
        for line in f:
            pass_dic.setdefault(line.strip().split()[0], line.strip().split()[-2])
    pdata2['color'] = pdata2['gene'].map(pass_dic).map({".": "blue", "PASS": "red"})
    ...
    axis.set_ylim(-0.1, 2 * 2 ** cnarr1.data[cnarr1.data['color'] == 'red']['median'].max() + 1)
    # Plot target region log2 ratio
    axis.scatter(cnarr1['id'], cnarr1['tumor'], color='#808080', edgecolor='none',
                 alpha=0.8, marker='o')
    cnarr1 = cnarr1[cnarr1['gene'].str.len() > 1]
    seg_lines = []
    for gene, subdata in cnarr1.data.groupby("gene", sort=False):
        line_color = (subdata['median'].median() > 0 and list(subdata['color'])[0] == 'red' and '#ff4500'
                      ) or (
                     subdata['median'].median() < 0 and list(subdata['color'])[0] == 'red' and '#7cfc00') or "#555555"
        seg_lines.append((subdata['median'].median(), subdata['id'].min(), subdata['id'].max(), line_color))
    # Plot gene log2 ratio
    for line in seg_lines:
        y1, x1, x2, segment_color = line
        y1 = 2 * 2 ** y1
        axis.plot((x1, x2), (y1, y1),
                  color=segment_color, linewidth=4, solid_capstyle='round')
    plt.tight_layout()
    plt.savefig(sample_name+'_CopyNumber_scatter.png', dpi=300)
